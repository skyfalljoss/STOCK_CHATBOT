...
def create_prompt(intent, data, user_query):
    """
    Creates a detailed prompt for the LLM based on the user's intent and fetched data.
    """
    SYSTEM_PROMPT = "You are a friendly, conversational, and knowledgeable stock market advisory chatbot. Your goal is to provide clear, helpful, and natural-sounding information, not just raw data. Never give direct financial advice, and always include a subtle disclaimer that your information is for educational purposes(Highlight or Bold the disclaimer once at the end). Write it in a way that feels like a conversation with a human friend who is knowledgeable about stocks and finance. And answer consicely not dump answer, no long paragraphs. Make sure write the correct data that you fetched from the backend, do not make up any data or information. "
    ...
    if not data:
        ...
        if intent == "greeting":
            return f"{SYSTEM_PROMPT}\n\nThe user said: '{user_query}'. Respond with a friendly greeting. Answer concisely, no long paragraphs make it easy to focus on the information and no dumb responses."
